...
```bash
docker run --rm ghcr.io/whisper-money/whisper-money:latest php artisan key:generate --show
```

Add the output to your `.env` file as `APP_KEY`.
...
> **Note**: For production deployments with external MySQL, remove the `mysql` service from the compose file and update `DB_HOST` in your `.env` to point to your database server.
...
|----------|-------------|
| `APP_KEY` | Laravel encryption key (auto-generated by Coolify) |
| `APP_URL` | Your application URL (auto-configured by Coolify) |
