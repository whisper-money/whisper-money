...
/**
 * The pictogram inside a medal, as SVG geometry.
 *
 * A shareable card is photographed from a Blade view, which cannot import the
 * `lucide-react` icons the screen uses, so the paths are copied here from
 * `node_modules/lucide-react/dist/esm/icons/*` verbatim — same 24-unit grid,
 * same 2-unit stroke. The keys are the `icon` names in {@see Catalog}.
 *
 * The summary cards solved the same problem the same way; see
 * `resources/views/cards/partials/icon-*.blade.php`.
 *
 * When a medal gains an icon the catalog knows and this map does not, the card
 * falls back to `target`, exactly as the screen does.
 */
class Pictograms
{
    private const PATHS = [
        ...
        'layers' => '<path d="M13 13.74a2 2 0 0 1-2 0L2.5 8.87a1 1 0 0 1 0-1.74L11 2.26a2 2 0 0 1 2 0l8.5 4.87a1 1 0 0 1 0 1.74z"/><path d="m20 14.285 1.5.845a1 1 0 0 1 0 1.74L13 21.74a2 2 0 0 1-2 0l-8.5-4.87a1 1 0 0 1 0-1.74l1.5-.845"/>',
    ...
    public function path(?string $icon): string
    {
        return self::PATHS[$icon] ?? self::PATHS['target'];
    }
