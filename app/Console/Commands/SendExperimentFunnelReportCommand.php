...
class SendExperimentFunnelReportCommand extends Command
{
    ...
    protected $description = 'Post the subscription experiment funnel (per variant) to Discord';

    /**
     * What the AI summary is looking at, and which periods it must compare.
     */
    private const SUMMARY_CONTEXT = <<<'CONTEXT'
    The report is the subscription experiment: one row per variant, cumulative
    since the experiment started. Conv% (conversions over matured users) and
    ARPU are the comparable metrics; the absolute MRR, cost and margin totals
    scale with how many users have matured, which differs per variant by design.
    Monetary amounts are in cents of the report currency, and null means there
    is no data yet rather than zero. The report is posted every Monday: compare
    against the previous run to say what moved this week, and never call a
    winner the significance block does not support.
    CONTEXT;
    ...
    public function handle(): int
    {
        ...
        if ($report['startedAt'] === null || $report['variants'] === []) {
            $this->warn('Experiment not running — declare subscriptions.experiment.variants and set SUBSCRIPTION_EXPERIMENT_STARTED_AT to begin.');
    ...
    private function tableLines(array $report): array
    {
        ...
        foreach (array_keys($report['variants']) as $label) {
            $row = $report['variants'][$label];
    ...
    private function significanceLines(array $report): array
    {
        ...
        $result = $this->significance->compare($leader, $runnerUp, comparisons: count($arms));
        ...
        $lines[] = sprintf(
            'Test exacto de Fisher p=%.3f %s α=%.3f (Bonferroni×%d) -> %s.%s',
            $result['fisherP'], $result['significant'] ? '<' : '≥', $result['alpha'], count($arms),
    ...
    private function buildEmbed(array $report, ?string $summary): array
    {
        ...
        return [
            'title' => '🧪 Experimento de suscripción — embudo por variante',
            'description' => $summary !== null ? $summary."\n\n".$table : $table,
            'color' => 0xFEE75C,
            'fields' => [
                [
                    'name' => 'Inicio',
                    'value' => $report['startedAt']->copy()->locale('es')->translatedFormat('D, d M Y').sprintf(' · los nuevos registros se reparten a partes iguales entre las %d variantes.', count($report['variants'])),
                    'inline' => false,
                ],
                [
                    'name' => '📊 Significancia',
